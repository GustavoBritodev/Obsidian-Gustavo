 ...
 ### Os oito tipos primitivos no Java são:
...
## Na declaração de tipo float, precisa conter um F maiúsculo ou minúsculo na atribuição de valor à variável. O tipo long segue a mesma lógica, sendo necessário ter um L no final da atribuição.
